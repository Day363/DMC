    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                float influence = saturate(1 - dist / _InfluenceRadius);

                float finalAmplitude = _Amplitude * (1 + influence * _Boost);

                // 오브젝트마다 랜덤 offset
                float randomOffset = rand(unity_ObjectToWorld._m03_m13);

                // 바람 계산
                float wave = sin(_Time.y * _Speed + v.vertex.y * _Frequency + randomOffset * 10);
